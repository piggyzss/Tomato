 ...
 */
export async function createModel(onProgress: (percent: number) => void): Promise<any> {
  try {
    const modelClass =
 ...
 */
export async function sendChatMessage(session: any, userMessage: string): Promise<Message> {
  try {
    if (!session) throw new Error("No active chat session");

    const promptResponse = await session.prompt([{ role: "user", content: userMessage }]);
