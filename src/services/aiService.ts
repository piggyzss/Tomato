...
class CloudAISession implements IAISession {
    ...
    async *promptStreaming(input: string): AsyncIterable<string> {
        try {
            ...
            console.error('Cloud AI streaming error:', error)
            throw new Error('云端 AI 流式生成失败')
        }
    }
...
export class AIService {
     ...
     */
    private async createBuiltInSession(config?: AISessionConfig): Promise<IAISession> {
        if (!window.ai?.languageModel) {
     ...
     * 创建云端 AI 会话
     */
    private async createCloudSession(config?: AISessionConfig): Promise<IAISession> {
        if (!this.cloudClient) {
    ...
    async generate(
        ...
        const { session, provider } = await this.createSession(config, preferredProvider)
